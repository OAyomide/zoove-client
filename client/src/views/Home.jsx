    ...
    <div className='text-center text-white flex flex-col justify-between h-screen'>
      <div className='xl:mx-64'>
        ...
        <div className='flex flex-col'>
          ...
          <span className='font-normal text-base md:mx-10'>
            Find the link to a song on different streaming platforms with one URL.
          </span>
          <input
            type='search'
            name=''
            ...
            className='mt-5 mb-3 px-12 rounded-lg py-4
       bg-zoove_gray-200 text-zoove_gray-300 border-2 border-zoove_gray-300 font-sans text-sm md:mx-12 mx-3'
            onChange={(e) => setSrcTrack(e.target.value)}
          />
          <span className='text-zoove_gray-400 md:font-medium  mx-3 text-xxs md:text-xs text-left md:mx-12'>
            Please note that large playlists might mean longer loading time.
          </span>
          <button
            className='bg-zoove_blue-100  place-self-center px-12 py-3 my-5 rounded-lg text-sm'
            onClick={async (e) => await FetchData()}
          >
            Search
          </button>
        </div>
        {isEmpty(tracks) ? (
          ...
          <div className='text-left md:mx-12 mx-3 flex flex-row mb-5'>
        ...
        {showExportComponent ? (
          addedToPlaylist ? (
            <div className='flex flex-col mx-5 rounded-lg bg-gray-1100 mt-5 items-center '>
              <span className='text-center my-5 text-white text-sm'>
                Playlist "{exportedPlaylist}" has been created for you. You can now view from your{' '}
            ...
            <div className='flex flex-col mx-5 rounded-lg bg-gray-1100 mt-5 items-center '>
              <div className='flex mt-10 sm:mt-5 ml-2 flex-col pb-10 sm:pb-5 bg-gray-1100 rounded '>
                <span className='text-white my-3'>
                  Export this playlist to {platform ?? 'whatever platform you use.'}
                </span>
                <input
                  type='text'
                  name='playlist-name'
                  id=''
                  placeholder={exportedPlaylist}
                ...
                />
                <button
                  className='text-black bg-blue-400 py-1 px-4 border border-blue-400 mb-2 ml-2 mr-5 sm:mr-6 mt-2 rounded md:w-auto md:mr12 lg:w-auto xl:mr-20 xl:w-auto'
                  onClick={async (e) => await CreatePlaylist()}
                >
                  Add to playlist
                </button>
              </div>
            ...
            <div className='flex flex-col mx-5 rounded-lg bg-gray-1100 mt-5 items-center '>
              <span className='text-white'>
                Cannot export to platform. Please connect your platform to add playlist.
              </span>
            </div>
          )
        ) : null}
        {loading ? (
          <div className='text-white text-center mt-10 flex flex-row justify-center'>
            <Loader type='Rings' spinnerStyle={{ color: '#9f7aea' }} />
          </div>
        ) : isEmpty(tracks) && !isPlaylist ? (
          isError ? (
        ...
        {isPlaylist
          ? Object.keys(platformsTracks).map((x, y) => {
              return (
                <div
                  className='flex flex-col border-t-8  border-yellow-700 rounded-lg my-5 md:mx-12 mx-3 bg-zoove_gray-600 pb-5 justify-evenly'
                  key={y}
                >
                  <img
                    src={x === 'spotify' ? SpotifyLogo : DeezerLogo}
                    alt=''
                  ...
                  <div className='flex flex-row justify-between'>
                    <div className='flex flex-row'>
                      <img
                        src={playlistMeta?.playlist_cover}
                        alt=''
                        className='w-16 h-16 mx-3 rounded-lg md:h-24 md:w-24'
                      />
                      <div className='flex flex-col text-left mt-2 flex-no-wrap md:mt-6'>
                        ...
                        <span
                          className='w-32 md:w-full whitespace-no-wrap block overflow-hidden text-xs'
                          style={{ textOverflow: 'ellipsis' }}
                        >
                          Playlist by {playlistMeta?.owner?.name}
                        </span>
                      </div>
                    </div>
                    ...
                    <div className='flex flex-row mt-2'>
                      <div
                        className='flex flex-col bg-zoove_gray-500 items-center rounded-lg h-8 mx-1 w-10 md:h-16 md:w-24 cursor-pointer'
                        onClick={(e) => togglePlaylistDropdown(x)}
                      >
                        <ViewIcon className='my-3 w-4 h-4' />
                        <span className='text-xxs mx-2 lg:flex md:flex md:text-xs sm:hidden'>
                          View
                        </span>
                      </div>
                      {playlistMeta?.playlist_url.includes(x) ? (
                        <div className='flex flex-col bg-zoove_gray-500 items-center rounded-lg h-8 mx-1 w-10 md:h-16 md:w-24 cursor-pointer'>
                          <CopyToClipboard
                            text={playlistMeta?.playlist_url}
                      ...
                      {playlistMeta?.playlist_url.includes(x) ? (
                        <div
                          className='flex flex-col bg-zoove_gray-500 items-center rounded-lg h-8 mx-1 w-10 md:h-16 md:w-24 cursor-pointer'
                          onClick={(e) => window.open(playlistMeta?.playlist_url)}
                        >
                          ...
                          <div className='flex-col flex' key={k + 384344}>
                            <div className='flex flex-row w-full md:hidden'>
                              <img
                                src={u?.cover}
                                alt=''
                                className='w-10 h-10 mx-3 rounded-lg md:h-12 md:w-12 ml-8 mt-2'
                              />
                              <div className='flex flex-row justify-between w-full mt-2 overflow-hidden text-left'>
                                <div style={{ width: '25%' }}>
                                  <span
                                    className='mt-3  text-xs whitespace-no-wrap block overflow-hidden'
                                ...
                                <span
                                  className='mt-3 text-xs whitespace-no-wrap block overflow-hidden font-light text-left'
                                  style={{ textOverflow: 'ellipsis', width: '25%' }}
                                >
                                  {u.artistes?.join(', ')}
                                </span>
                                <span
                                  className='mt-3 text-xs whitespace-no-wrap block overflow-hidden  text-right font-light'
                                  style={{ textOverflow: 'ellipsis', width: '25%' }}
                                >
                                  {ConvertToMusicDuration(u?.duration)}
                                </span>
                                <div
                                  className='mt-3 flex flex-col  items-center rounded-lg h-8 w-10 md:h-16 md:w-24'
                                  style={{ maxWidth: '25%' }}
                                >
                                  <ReactAudio
                                    ref={(input) => {
                                  ...
                                  <PlayIcon
                                    ...
                                    onClick={(e) => handlePreviewClick(u?.preview, k)}
                                  />
                                </div>
                              </div>
                            </div>
                            ...
                            <div className='flex flex-row w-full sm:hidden lg:flex md:flex xl:flex mt-2'>
                              ...
                              <div className='flex flex-row justify-between w-full mt-2 overflow-hidden text-left'>
                                ...
                                <span
                                  className='mt-3 text-xs whitespace-no-wrap block overflow-hidden font-light text-pink-1000 italic'
                                  style={{ textOverflow: 'ellipsis', width: '20%' }}
                                >
                                  {u?.album ?? 'Unavailable'}
                                </span>
                                <span
                                  className='mt-3 text-xs whitespace-no-wrap block overflow-hidden font-light italic'
                                  style={{ textOverflow: 'ellipsis', width: '5%' }}
                                >
                                  {formatReleaseDate(u?.release_date)}
                                </span>
                                <span
                                  className='mt-3 text-xs whitespace-no-wrap block overflow-hidden  text-right font-light'
                                  style={{ textOverflow: 'ellipsis', width: '8%' }}
                                >
                                  {ConvertToMusicDuration(u?.duration)}
                                </span>
                                <div
                                  className='flex flex-col  items-center rounded-lg h-8 w-10 md:h-16 md:w-24'
                                  style={{ width: '8%' }}
                                  onClick={(e) => handlePreviewClick(u?.preview, k)}
                                >
                                  <ReactAudio
              ...
              return (
                <div key={y + 100}>
                  {x.map((i, j) => {
                    ...
                    return (
                      <div
                        className='flex flex-col border-t-8  border-yellow-700 rounded-lg my-5 md:mx-12 mx-3 bg-zoove_gray-600 pb-5 '
                        key={j}
                      >
                        <img
                          src={x[j]?.platform === 'spotify' ? SpotifyLogo : DeezerLogo}
                          alt=''
                          className='w-16 h-16 mx-3'
                          onClick={(e) => {
                            window.open(x[j]?.url);
                        ...
                        <div className='flex flex-row justify-between'>
                          <div className='flex flex-row'>
                            ...
                            <div className='flex flex-col text-left mt-2 md:mt-6 flex-no-wrap'>
                              <span className='text-sm text-pink-1000 md:text-base'>
                                {x[j]?.title}
                              </span>
                              <span
                                className='w-32 md:w-full whitespace-no-wrap block overflow-hidden text-xs'
                          ...
                          <div className='flex flex-row mt-2'>
                            ...
                            <div className='flex flex-col bg-zoove_gray-500 items-center rounded-lg h-8 mx-1 w-10 md:h-16 md:w-24 cursor-pointer'>
                              <OpenLinkIcon className='my-3 w-4 h-3 md:w-6 md:h-4' />
                              <span
                                className='text-xxs mx-2 lg:flex md:flex md:text-xs sm:hidden'
                                onClick={(e) => window.open(x[j]?.url)}
                              >
                                Open link
                              </span>
                            </div>
                            ...
                            <div className='flex flex-col bg-zoove_gray-500 items-center rounded-lg h-8 mx-1 w-10 md:h-16 md:w-24 cursor-pointer'>
                              <PlayIcon
                                className='my-3 w-4 h-3 md:w-6 md:h-4'
                                onClick={(e) => handlePreviewClick(previewURL, j)}
                              />
                              <span className='text-xxs mx-2 lg:flex md:flex md:text-xs sm:hidden cursor-pointer'>
                                Preview
                              </span>
                            </div>
                          </div>
      ...
      <span className='my-5'>
        😚 zoove {new Date().getFullYear()}.{' '}
        <span className='text-yellow-1000 cursor-pointer' onClick={(e) => {}}>
          <a href='mailto:onigbindeayomide@gmail.com?subject=Hi! I want to talk to you about zoove'>
